...
En choisissant un seuil NIR_humide > 0.35, on obtient les zones avec un NIR très élevé (cultures à ciel ouvert et agroforesteries sous canopée). Pour n'obtenir que les agroforesteries sous canopée, on peut discriminer les champs ouverts en choisissant un seuil RED_humide > 0.04  (les images en saison humide étant plus propres).
Aussi, on pourrait aussi appliquer un seuil NIR_sec pour obtenir directement les agroforesteries sous canopée bien que moins visibles (d'où l'utilité du NIR_humide). 
...
# Méthode 3: Méthode SBR FAR EDGE

## Raisonnement

Les bandes du domaine Red Edge de Sentinel-2 sont sensibles aux variations de teneur en chlorophylle, de structure foliaire et de densité du couvert végétal. Le rapport B6/B5 permet ainsi de mieux caractériser les différences physiologiques et structurales entre une forêt dense et une agroforêt, notamment lorsque les deux présentent des valeurs de NDVI élevées et proches. Les agroforêts sous ombrage possèdent toutefois des réflectances dans le Red Edge remarquables, plus élevées que celles de la forêt dense: on s'attend donc à obtenir pour ces espaces, un rapport Red Edge2/Red Edge1 faible. Le rapport B7/B5 serait égelement intéressant...
...
// == Masque nuages Sentinel-2 avec SCL ==
...
var imagesMasked = images.map(maskSentinel2);


// == Ajout du SBR (ici SBR = RED EDGE 2 / RED EDGE 1 = B6 / B5) ==
...
var imagesSBR = imagesMasked.map(addSBR);
...
print(
  'Dates :',
  imagesSBR.aggregate_array('system:time_start')
);

print(
  'Produits :',
  imagesSBR.aggregate_array('PRODUCT_ID')
);
...
// == Export complet (Rouge + Red Edge + SWIR + SBR) ==

var compositeExport = imagesSBR
  .median()
  .select([
    'B4',     // Rouge
    'B5',     // Red Edge 1
...
Export.image.toDrive({
  ...
  image: compositeExport,

  description:
  'Sentinel2_Red_RE_SWIR_SBR_Median_2021_2026',
...
Export.image.toDrive({
  ...
  image: rededge1Median,

  description:
  'RED_EDGE1_Median_2021_2026',
...
Le script précédent donne un composite composé de composites médians sur la période 2021-2026. Il intègre plusieurs bandes sensibles aux caractéristiques de la végétation :

B4 (rouge) : absorption chlorophyllienne ;
B5 et B6 (bandes red edge) : structure du couvert végétal et variations de biomasse ;
B8 (proche infrarouge) : vigueur et densité de la végétation ;
B11 (SWIR1) : humidité et structure du couvert.
